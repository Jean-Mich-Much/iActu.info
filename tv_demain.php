...
if(!file_exists($nom_page.'.html')||filemtime($nom_page.'.html')<(time()-$cache_s)||!file_exists($nom_page.date("j",strtotime("+1 day")).'.html')){ob_start();?>
<!DOCTYPE html>
<html lang="fr">
 ...
 <body id="body" lang="fr">
  <div class="flex-page">
   ...
   <div class="mid"><?php $xmlFile="Structure/cache/tv/xmltv_tnt.xml";$tem="Structure/cache/tv/xmltv_tnt".date("j",strtotime("+1 day")).".tem";$cacheSTv=18000;if(!file_exists($xmlFile)||@filemtime($tem)<time()-$cacheSTv){exec('wget -O "/_/Structure/cache/tv/xmltv_tnt.xml" "https://xmltvfr.fr/xmltv/xmltv_tnt.xml" -q --no-check-certificate');if(!file_exists("Structure/cache/tv/xmltv_tnt.time")){fopen("Structure/cache/tv/xmltv_tnt.time","w");}@copy("Structure/cache/tv/xmltv_tnt.time",$tem);}$xml=new DOMDocument();if(!$xml->load($xmlFile)){die("Erreur de chargement du fichier XML");}$xp=new DOMXPath($xml);$chs=$xp->query("/tv/channel");$td=(new DateTime('tomorrow'))->format("Y-m-d");$tm=(new DateTime('tomorrow'))->modify("+1 day")->format("Y-m-d")." 06:00:00";function getCatEmoji($cat){$e=["Film"=>"🎞️ Film","Mag"=>"📰 Mag","Jeunesse"=>"🧸 Jeunesse","Info"=>"ℹ️ Info","Div"=>"🎉 Div","Prog"=>"📺 Prog","Var"=>"🎶 Var","Spec"=>"🎭 Spec","PopRock"=>"🎸 PopRock","default"=>"📺 Autre",];return$e[$cat]??$e["default"];}foreach($chs as $ch){$chId=$ch->getAttribute("id");$dispName=$ch->getElementsByTagName("display-name")[0]->nodeValue;$ico=$ch->getElementsByTagName("icon")[0]?$ch->getElementsByTagName("icon")[0]->getAttribute("src"):"";
...
$firstStop=$firstPT?new DateTime($firstPT->getAttribute("stop"),new DateTimeZone("Europe/Paris")):null;
$secondPTStart=($firstStop&&$firstStop>$ptEnd)?$firstStop:new DateTime("$td 21:29:59",new DateTimeZone("Europe/Paris"));
...
foreach([$firstPT,$secondPT]as$p){
if($p&&!in_array($p->getAttribute("start").$p->getAttribute("stop").$p->getElementsByTagName("title")[0]->nodeValue,$shown)){
    $shown[]=$p->getAttribute("start").$p->getAttribute("stop").$p->getElementsByTagName("title")[0]->nodeValue;
    $start=new DateTime($p->getAttribute("start"),new DateTimeZone("Europe/Paris"));
    $stop=new DateTime($p->getAttribute("stop"),new DateTimeZone("Europe/Paris"));
    $title=$p->getElementsByTagName("title")[0]?$p->getElementsByTagName("title")[0]->nodeValue:"";
    $cat=$p->getElementsByTagName("category")[0]?$p->getElementsByTagName("category")[0]->nodeValue:"default";
    $catEmoji=getCatEmoji($cat);
    $dur=$start->diff($stop);
    $desc=$p->getElementsByTagName("desc")[0]?$p->getElementsByTagName("desc")[0]->nodeValue:"";
    $ico=$p->getElementsByTagName("icon")[0]?$p->getElementsByTagName("icon")[0]->getAttribute("src"):"";
    echo"<div style='display:flex;flex-direction:column;width:100%;padding: round(calc(8 / 1920 * 100vw), 1px) round(calc(8 / 1920 * 100vw), 1px) round(calc(12 / 1920 * 100vw), 1px) round(calc(8 / 1920 * 100vw), 1px);margin:round(up,calc(4 / 1920 * 100vw),1px) round(calc(1 / 1920 * 100vw),1px) round(up,calc(4 / 1920 * 100vw),1px) auto;border-bottom:round(up,calc(1 / 1920 * 100vw),1px) solid var(--c7);'>
        <div style='align-self:flex-start;margin-bottom: round(calc(8 / 1920 * 100vw), 1px);margin-top: round(calc(1 / 1920 * 100vw), 1px);'>
            <strong class='f18px' style='color: var(--c5);font-variation-settings: \"wght\" 500;'>🎬&nbsp;$title</strong>
        </div>";
    echo "<div style='align-self:flex-start;'>⌚&nbsp;{$start->format("H:i")}&nbsp;-&nbsp;$catEmoji</div>";
    echo "<div style='align-self:flex-start;'><img src='$ico' alt='$title' style='outline: round(calc(3 / 1920 * 100vw), 1px) solid var(--c8);outline-offset: round(calc(-3 / 1920 * 100vw), 1px);border-radius: round(calc(12 / 1920 * 100vw), 1px);margin: round(calc(8 / 1920 * 100vw), 1px) 0;min-width:auto;min-height: auto;max-width: 75%;max-height: round(calc(320 / 1920 * 100vw), 1px);width: auto;height: auto;'></div>";
    echo "<div><strong style='line-height: round(calc(24 / 1920 * 100vw), 1px);'>⏰&nbsp;</strong>{$dur->h}h{$dur->i}min<br><strong>📜&nbsp;</strong>$desc</div></div>";}}echo "</div>";};
?></div>
</div>
