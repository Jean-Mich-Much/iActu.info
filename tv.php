...
foreach($chs as $ch){
    ...
    $dispName = $ch->getElementsByTagName("display-name")[0]->nodeValue ?? "Nom indisponible";
    $ico = $ch->getElementsByTagName("icon")[0]->getAttribute("src") ?? "";
    ...
    if ($firstPT) {
        $firstStop = new DateTime($firstPT->getAttribute("stop"), new DateTimeZone("Europe/Paris"));
        ...
        foreach ($progs as $p) {
            ...
            if ($start >= $secondPTStart && $start <= $secondPTEnd && $durMin > 24) {
                if (!$secondPT) {
                    $secondPT = $p;
                } elseif ($start > $firstStop && $start >= $secondPTStart && $durMin > 24) {
                    $secondPT = $p;
                    break;
                }
            }
        ...
        if (!$secondPT) {
            foreach ($progs as $p) {
                ...
                if ($start > $firstStop && $durMin > 24 && !in_array($p->getAttribute("start").$p->getAttribute("stop").$p->getElementsByTagName("title")[0]->nodeValue, $shown)) {
                    $secondPT = $p;
                    break;
                }
            }
        ...
        foreach ([$firstPT, $secondPT] as $p) {
            if ($p && !in_array($p->getAttribute("start").$p->getAttribute("stop").$p->getElementsByTagName("title")[0]->nodeValue, $shown)) {
                ...
                $start = new DateTime($p->getAttribute("start"), new DateTimeZone("Europe/Paris"));
                $stop = new DateTime($p->getAttribute("stop"), new DateTimeZone("Europe/Paris"));
                $title = $p->getElementsByTagName("title")[0]->nodeValue ?? "Titre indisponible";
                $cat = $p->getElementsByTagName("category")[0]->nodeValue ?? "default";
                $catEmoji = getCatEmoji($cat);
                $dur = $start->diff($stop);
                $desc = $p->getElementsByTagName("desc")[0]->nodeValue ?? "aucune description";
                $ico = $p->getElementsByTagName("icon")[0]->getAttribute("src") ?? "";
                ...
                echo "<div style='align-self:flex-start;margin-bottom: round(calc(8 / 1920 * 100vw), 1px);margin-top: round(calc(1 / 1920 * 100vw), 1px);'><strong class='f18px' style='color: var(--c5);font-variation-settings: \"wght\" 500;'>🎬&nbsp;$title</strong></div>";
                ...
                echo "<div><strong style='line-height: round(calc(24 / 1920 * 100vw), 1px);'>⏰&nbsp;</strong>{$dur->h}h{$dur->i}min<br><strong>📜&nbsp;</strong>$desc</div></div>";
            }
        }
    }
    echo "</div>";
