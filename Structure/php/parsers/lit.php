...
$result=implode('',array_map(fn()=>$c[array_rand($c)],range(1,$a)));return (string)$result;}
...
try {$pubDate = new DateTime($pubDateString);}
catch (Exception) {$pubDate = (clone $currentDateTime)->modify('-1 day');}
if ($pubDate > $currentDateTime) {$pubDate = (clone $currentDateTime)->modify('-1 day');}
$convertedTitle = mb_convert_encoding($title, 'UTF-8', 'auto') ?: $title;
if (mb_check_encoding($convertedTitle, 'UTF-8') === false) {continue;}
$title = preg_replace('/\s+/', ' ', $convertedTitle);
$validLink = filter_var($link, FILTER_VALIDATE_URL) !== false;
$key = $title . $link;
if (!isset($uniqueItems[$key]) && strlen($title) >= 8 && strlen($link) >= 8) {
if (!in_array($link, $seenLinks) &&
(!in_array($title, $seenTitles) || strlen($title) <= 32)) {
$itemData = [
'title' => $title,
'link' => $link,
'pubDate' => $pubDate->getTimestamp(),
];
if ($validLink) {$uniqueItems[$key] = $itemData;}
else {$fallbackItems[$key] = $itemData;}
$seenLinks[] = $link;
if (strlen($title) > 32) {$seenTitles[] = $title;}}}}
usort($uniqueItems, fn($a, $b) => $b['pubDate'] - $a['pubDate']);
...
$itemDate=DateTime::createFromFormat('U',$item['pubDate']);
if($itemDate>=$todayStart&&count($todayNews)<20){$todayNews[]=$item;}
elseif(count($otherNews)<20){$otherNews[]=$item;}
if(count($todayNews)+count($otherNews)>=20){break;}}
...
foreach($news as $item){$id=generateUniqueId();
if($isToday){$id=date('Y-z').'-'.$id;}
...
echo'<!-- FIN NEWS -->';$p=ob_get_clean();
usleep(10);
if(substr_count($p,'<!-- FIN NEWS -->')===1&&substr_count($p,'href="http')>=20&&strlen($p)>=768){
...
<?php
if (file_exists($dossier . $fichier . '.' . date('yz'))) {$fichiero = @file_get_contents($dossier . $fichier . '.' . date('yz'));
echo substr_count($fichiero, 'id="' . date('Y-z') . '-') . " " . $texte1;} else echo $texte2;
?>
</span>
</div>
<div class="site_news">
<?php
if (file_exists($dossier . $fichier . '.' . date('yz'))) {$fichiera = @file_get_contents($dossier . $fichier . '.' . date('yz'));
echo str_replace($texte3, $texte4, str_replace($texte5, $texte6, $fichiera));} else {$fichierb = @file_get_contents($dossier . $fichier . '.html');
